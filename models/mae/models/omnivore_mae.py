...
class OmniMAE(nn.Module):
    ...
    def forward(self, imgOrVideo, mask=None, **kwargs):
        ...
        outputs = self.trunk(imgOrVideo, mask=mask, **kwargs)
...
def vit_base_mae_pretraining(pretrained=True, n_frame=2):
    trunk = VisionTransformer(
        img_size=[3, n_frame, 224, 224],
        patch_size=[2, 16, 16],
        in_chans=3,
        embed_dim=768,
        depth=12,
        ...
        patch_embed_params_list=[
            PadIm2Video(ntimes=1, pad_type="repeat"),
            make_conv_or_linear(
...
def vit_large_mae_pretraining(pretrained=True, n_frame=2):
    trunk = VisionTransformer(
        img_size=[3, n_frame, 224, 224],
        patch_size=[2, 16, 16],
        in_chans=3,
