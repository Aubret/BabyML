...
for t in textures_wanted.keys():
    ...
    if not os.path.exists(f"resources/textures/{tmap}"):
        os.makedirs(f"resources/textures/{tmap}")
    for i, image in enumerate(os.listdir(d)):
        if not os.path.exists(f"resources/textures/{tmap}/{image}"):
            try:
                img = PIL.Image.open(os.path.join(d, image))
            except:
                continue
            img = torchvision.transforms.functional.center_crop(img, (128,128))
            img = torchvision.transforms.functional.resize(img, (224,224), interpolation=InterpolationMode.BICUBIC)
            img.save(f"resources/textures/{tmap}/{image}")
        i += 1
        if i > 12:
            break
