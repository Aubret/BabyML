...
def invariance_v2(args):
    ...
    dataset_pos = DATASETS[args.dataset](args.data_root, subset_name=args.pos_subset, transform=preprocess, whitebg=whitebg, rotation=["50"])
    dataset_neg = DATASETS[args.dataset](args.data_root, subset_name=args.neg_subset, transform=preprocess, whitebg=whitebg, rotation=["100"])
    ...
    for id in range(1, 48, 1):
        ...
        d1 += torch.nn.functional.cosine_similarity(features, featuresp, dim=1).sum().item()
        d2 += torch.nn.functional.cosine_similarity(features, featuresn, dim=1).sum().item()

        d3 += torch.norm(features - featuresp, dim=1).sum().item()
        d4 += torch.norm(features - featuresn, dim=1).sum().item()

    return np.array([success, d1, d2, d3, d4])/cpt
...
if __name__ == '__main__':
    ...
    args.log_dir = os.path.join(args.log_dir, args.dataset, "invcheck")
    if args.dense_features:
        args.log_dir += "dense"
